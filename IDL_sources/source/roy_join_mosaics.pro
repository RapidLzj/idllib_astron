...
;; median). THIS NEEDS TO BE IMPROVED!!
  ...
  first=0 ;; if set to 1, the fits files are read, aligned and put in a cube.
...
;;      SHOULD DROP OUT (background subtraction)!
        ...
        n1=round(0.2*n_elements(pixelvector)) & n2=round(0.6*n_elements(pixelvector))
...
;          read,aa
        ...
        print,'running a median filter across the different mosaics, making the final combined image ...'
        stack=median(cube.im,dimension=3)
        outfile=work_directory+'joined_IRAC_mosaics/'+field+'_'+bands[iband]+'_stack'+shortlong+'.fits'
