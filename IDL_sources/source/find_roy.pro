pro find_roy, image, x, y, flux, sharp, roundness, hmin, fwhm, roundlim, sharplim,$
...
SEARCH: 			    ;Threshold dependent search begins here
 ...
 index = where(((h GE hmin) and (finite(h) eq 1)), nfound)  ;Valid image pixels are greater than hmin

 if (nfound[0] EQ 0) then begin          ;Any maxima found?
...
;        if nfound LT 0 then begin  ;Do valid local maxima exist?
        if (stars[0] eq -1) then begin  ;Do valid local maxima exist?
             message,'ERROR - No maxima exceed input threshold of ' + $
                     string(hmin,'(F9.1)'),/CON
             goto,FINISH  
        endif
...
;     temp = float(image[ix[i]-nhalf:ix[i]+nhalf,iy[i]-nhalf:iy[i]+nhalf])
     temp = float(image[(ix[i]-nhalf)>0:(ix[i]+nhalf)<(n_x-1),(iy[i]-nhalf)>0:(iy[i]+nhalf)<(n_y-1)])
     d = h[ix[i],iy[i]]                  ;"d" is actual pixel intensity        

;print,(ix[i]-nhalf)>0,':',(ix[i]+nhalf)<(n_x-1),',',(iy[i]-nhalf)>0,':',(iy[i]+nhalf)<n_y-1,iy[i],nhalf
;print,'api: ',n_elements(temp),(2*nhalf+1)^2,nhalf

     npixtemp=n_elements(temp)
     if (npixtemp lt (2*nhalf+1)^2) then begin ;; too close to the edge of a sub-image,
         ;; we will catch this one in another sub-image due to the overlap,
         ;; which is chosen large enough!
       badsharp=badsharp+1
       goto, REJECT
     endif
