...
PRO simple_psf_extract, $ 
	x, y, image, psf_size, psf, psf_fwhm, background,$ 
	x_secondary=x_secondary, y_secondary=y_secondary, $  
	...
	if  n_elements(upper_lev) ne 0  then begin 
	   ...
	   x_unsat = x_unsat[w]  &  y_unsat = y_unsat[w]  &  peaks = peaks[w] 
        ...
        if keyword_set(x_secondary) then begin 
...
;	 Select the brightest unsaturated star 
	...
	R2=psf_size*4 
        ...
        for i=0L,n_elements(x_unsat)-1L do begin 
	skyval=sky_med(image,x_unsat[i],y_unsat[i],R1,R2) 
	lx=round(x_unsat[i]-R2) 
        ly=round(y_unsat[i]-R2) 
        ux=round(x_unsat[i]+R2) 
        uy=round(y_unsat[i]+R2) 
        background[lx:ux,ly:uy]=skyval 
	endfor 
	 
	 
	psf = superpose_stars(image - background, x_unsat, y_unsat, $ 
	                         psf_size, RAD_NORM = rad, CENTROID_BOX = round(psf_fwhm), $ 
	                         _EXTRA = extra) 
