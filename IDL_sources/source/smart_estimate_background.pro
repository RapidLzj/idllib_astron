...
FUNCTION smart_estimate_background, image, step, starlist=starlist, starbox=starbox,SKY_MEDIAN = sky_median, _EXTRA = extra

	on_error, 2
	
	if not keyword_set(starlist) then begin
	
	if  keyword_set(sky_median)		   then $
   	   b = median_filter(image, step)  else $
   	begin
	   b = smart_image_background(image, step, _EXTRA = extra)
	...
	if not keyword_set(starbox) then starbox=3.0 
	...
	if  keyword_set(sky_median)		   then $
	   ...
	   b = smart_image_background(image, step,starlist=starlist,starbox=starbox,_EXTRA = extra)
	   if  n_elements(b) eq 1  then begin
	      message, 'unable to estimate background; trying with median smoothing', /INFO
	      b = median_filter(image, step)
	   endif
