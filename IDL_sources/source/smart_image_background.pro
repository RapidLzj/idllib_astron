...
FUNCTION smart_image_background, image, sampling_step, starlist=starlist,starbox=starbox,$
	...
	stars=fltarr(sx,sy)+1.0
	
	if keyword_set(starlist) and n_elements(starlist) gt 0 then begin
	
	for j=0LL,n_elements(starlist)-1LL do begin
	lx=round(starlist[j].x-starbox/2)
	ly=round(starlist[j].y-starbox/2)
	ux=round(starlist[j].x+starbox/2)
	uy=round(starlist[j].y+starbox/2)
	stars[lx:ux,ly:uy]=!VALUES.F_NAN
	endfor
        endif
