...
;; purpose: to find all the sub-images that cover the [x,y] position
  ...
  if (n_elements(ix) gt 4) then ix=ix[0:3]
...
pro ssf_smart_source_finder_psf,field=field,band=band,ds9=ds9,$
...
;        nsig_limslim=15.
  ...
  if not keyword_set(infile) then $
   ...
   if band eq '3.6' then infile='/home/fang/disk2/PISMIS24/image/GLM1_psf.fits'
   if band eq '4.5' then infile='/home/fang/disk2/PISMIS24/image/GLM2_psf.fits'
   if band eq '5.8' then infile='/home/fang/disk2/PISMIS24/image/GLM3_psf.fits'
   if band eq '8.0' then infile='/home/fang/disk2/PISMIS24/image/GLM4_psf.fits'
...
;  cmd=cmd+' &'
 ...
 ;if band ne '24' then begin
  ...
  outfile=work_directory+'joined_IRAC_mosaics/found_sources/GLM_psf_'+field+'_'+band+'_sources'
